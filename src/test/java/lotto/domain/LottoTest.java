...
class LottoTest {
    ...
    @Test
    void 로또_객체생성_테스트() {
        assertThatCode(() -> Lotto.buildLotto(List.of(1, 2, 3, 4, 5, 6)))
    ...
    void 로또_번호_개수초과_예외_테스트() {
        assertThatThrownBy(() -> Lotto.buildLotto(List.of(1, 2, 3, 4, 5, 6, 7)))
    ...
    void 로또_번호_범위밖_예외_테스트(List<Integer> numbers) {
        assertThatThrownBy(() -> Lotto.buildLotto(numbers))
                .isInstanceOf(IllegalArgumentException.class);
    }
    ...
    @Test
    void 로또_정렬_테스트() {
        Lotto lotto = Lotto.buildLotto(List.of(1, 2, 32, 4, 3, 5));
